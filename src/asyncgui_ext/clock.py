...
def move_on_after(clock: Clock, timeout) -> AbstractAsyncContextManager[Task]:
    ...
    '''
    return wait_any_cm(clock.sleep(timeout))
...
async def interpolate_scalar(clock: Clock, start, end, *, duration, step=0, transition=Transition.linear) -> AsyncIterator:
    '''
...
async def interpolate_sequence(clock: Clock, start, end, *, duration, step=0, transition=Transition.linear,
...
def anim_attrs(clock: Clock, obj, *, duration, step=0, transition=Transition.linear, output_seq_type=tuple,
...
def anim_attrs_abbr(clock: Clock, obj, *, d, s=0, t=Transition.linear, output_seq_type=tuple,
                    **animated_properties) -> Awaitable:
    '''
