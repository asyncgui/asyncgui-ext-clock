...
def test_scalar(clock):
    ...
    import asyncgui
    from asyncgui_ext.clock import anim_attrs

    obj = SimpleNamespace(num=0)
    task = asyncgui.start(anim_attrs(clock, obj, num=20, duration=100))
...
def test_sequence(clock):
    ...
    task = asyncgui.start(anim_attrs(clock, obj, pos=[100, 0], duration=100))
...
def test_seq_type_parameter(clock, output_seq_type):
    ...
    task = asyncgui.start(anim_attrs(clock, obj, size=[10, 10], pos=(10, 10), duration=10, output_seq_type=output_seq_type))
    clock.tick(0)
